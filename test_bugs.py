...
ok, msg, area_id = db.add_cave_area('TEST', '测试洞区', '测试位置', '花岗岩', '测试用')
...
records = db.get_qc_records()
print(f'get_qc_records (no filter): {len(records)} records')
records = db.get_qc_records(drip_point_id=point_id)
print(f'get_qc_records (point filter): {len(records)} records')
records = db.get_qc_records(batch_id=None, drip_point_id=point_id)
print(f'get_qc_records (batch=None, point=id): {len(records)} records')
